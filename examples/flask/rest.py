...
app = Flask(
    __name__, template_folder=tmpl_dir, static_folder=static_dir, static_url_path=""
)
...
class SoundCloudDownloader(Resource):
    ...
    def get(self, inputname):
        ...
        PLD_soundcloud.download_playlist(
            soundcloud_list, "download/soundcloud_%d/" % self._ids, compress=COMPRESSION
        )
...
class YoutubeDownloader(Resource):
    ...
    def get(self, inputname):
        ...
        PLD_youtube.download_playlist(
            youtube_list, "download/youtube_%d/" % self._ids, compress=COMPRESSION
        )
...
class SpotifyDownloader(Resource):
    ...
    def get(self, inputname):
        ...
        PLD_spotify.download_playlist(
            spotify_list, "download/spotify_%d/" % self._ids, compress=COMPRESSION
        )
...
class Downloader(Resource):
    ...
    def get(self, inputname):
        ...
        PLD = PlaylistDownloader(
            spotipyid=SPOTIPYCLIENTID, spotipysecret=SPOTIPYCLIENTSECRET
        )
        playlist = PLD.load_playlist(f"{UPLOAD_FOLDER}/{inputname}")
        PLD.download_playlist(
            playlist, "download/downloader_%d" % self._ids, compress=COMPRESSION
        )
...
class UploadFile(Resource):
    ...
    def post(self):
        ...
        return {"message": "Invalid filename or extension (jpg, png, gif)"}
